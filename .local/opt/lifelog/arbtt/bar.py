...
def preprocess(category, table, args):
    ...
    return Preprocessed(
        category=category, time_sum=time_sum, table=table, table_totals=table_totals
    )
...
def prepare_one_bartable(time_total, prep, args):
    ...
    table["FracAbove"] = (
        table["Frac"].shift(1, fill_value=0).cumsum() if args.stacked else 0
    )
...
def prepare_bartables(inputs, args):
    preprocessed = [
        preprocess(category, table, args=args) for category, table in inputs.items()
    ]
